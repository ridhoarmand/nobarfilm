...
function DramaSearchContent() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    router.push(`/drama/search?platform=${encodeURIComponent(activePlatform)}&q=${encodeURIComponent(searchQuery)}`);
  };
          ...
          {!isLoading && !error && query && (
            <>
              {items.length === 0 ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
                  {activePlatform === 'dramabox' && items.map((drama: any) => <DramaCard key={drama.bookId} drama={drama} />)}
                  ...
                  {activePlatform === 'melolo' &&
                    items.map((item: any) => (
                      <UnifiedMediaCard
                        ...
                        link={`/drama/melolo/${item.book_id}`}
                        episodes={item.serial_count || 0}
                      />
                    ))}
                  ...
                  {activePlatform === 'freereels' &&
                    items.map((item: any) => (
                      <UnifiedMediaCard
                        ...
                        link={`/drama/freereels/${item.key || item.id}`}
                  ...
                  {activePlatform === 'flickreels' &&
                    items.map((item: any) => (
                      <UnifiedMediaCard
                        ...
                        link={`/drama/flickreels/${item.id || item.playlet_id}`}
