...
function MeloloSearchContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const initialQuery = searchParams.get('q') || '';
  const [searchQuery, setSearchQuery] = useState(initialQuery);
  const { data, isLoading, error } = useMeloloSearch(initialQuery);

  const items = data?.data?.search_data?.[0]?.books || [];

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/drama/melolo/search?q=${encodeURIComponent(searchQuery)}`);
    }
  };
            ...
            <form onSubmit={handleSearch} className="max-w-2xl">
              <div className="relative">
                <input
                  ...
                  placeholder="Search Melolo dramas..."
...
export default function MeloloSearchPage() {
  return (
    <Suspense fallback={
      <>
        <Navbar />
