...
function ReelShortSearchContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const initialQuery = searchParams.get('q') || '';
  const [searchQuery, setSearchQuery] = useState(initialQuery);
  const { data, isLoading, error } = useReelShortSearch(initialQuery);

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/drama/reelshort/search?q=${encodeURIComponent(searchQuery)}`);
    }
  };
            ...
            <form onSubmit={handleSearch} className="max-w-2xl">
              <div className="relative">
                <input
                  ...
                  placeholder="Search ReelShort dramas..."
              ...
              {data.data.length === 0 ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
                  {data.data.map((book: any) => (
                    <ReelShortCard key={book.book_id} book={book} />
                  ))}
                </div>
              )}
            </>
