...
import { Play, ChevronLeft, Info, Calendar } from 'lucide-react';
...
export default function ReelShortDetailPage() {
  ...
  const { data: homepageData, isLoading: trendingLoading } = useReelShortHomepage();

  const trendingDramas: Drama[] = useMemo(() => {
    if (!homepageData?.data?.lists) return [];
    const books = homepageData.data.lists.find((l) => l.books && l.books.length > 0)?.books || [];
    return books.slice(0, 16).map((book) => ({
      bookId: book.book_id,
            ...
            <div className="hidden md:block">
              <div className="relative aspect-[2/3] w-full rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10 group">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <Link
                    href={`/drama/watch/reelshort/${data.bookId}`}
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 gap-2">
                {Array.from({ length: data.totalEpisodes }, (_, i) => i + 1).map((ep) => (
                  <Link
                    key={ep}
                    href={`/drama/watch/reelshort/${bookId}?ep=${ep}`}
            ...
            <section className="bg-white/5 rounded-2xl p-6 border border-white/10">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <span className="text-white font-medium">ReelShort Original</span>
                </div>
              </div>
            </section>
