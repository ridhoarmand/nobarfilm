...
function FreeReelsSearchContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const initialQuery = searchParams.get('q') || '';
  const [searchQuery, setSearchQuery] = useState(initialQuery);
  const { data, isLoading, error } = useFreeReelsSearch(initialQuery);
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/drama/freereels/search?q=${encodeURIComponent(searchQuery)}`);
    }
  };
            ...
            <form onSubmit={handleSearch} className="max-w-2xl">
              <div className="relative">
                <input
                  ...
                  placeholder="Search FreeReels dramas..."
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
                  {items.map((item: any) => (
                    <UnifiedMediaCard
                      ...
                      title={item.vodName}
                      cover={item.vodPic}
                      link={`/drama/freereels/${item.vodId}`}
                      episodes={item.vodEpisodeCount || 0}
                    />
                  ))}
                </div>
              )}
...
export default function FreeReelsSearchPage() {
  return (
    <Suspense fallback={
      <>
        <Navbar />
