...
function DramaBoxSearchContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const initialQuery = searchParams.get('q') || '';
  const [searchQuery, setSearchQuery] = useState(initialQuery);
  const { data, isLoading, error } = useSearchDramas(initialQuery);

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/drama/dramabox/search?q=${encodeURIComponent(searchQuery)}`);
    }
  };
            ...
            <form onSubmit={handleSearch} className="max-w-2xl">
              <div className="relative">
                <input
                  ...
                  placeholder="Search DramaBox dramas..."
...
export default function DramaBoxSearchPage() {
  return (
    <Suspense fallback={
      <>
        <Navbar />
