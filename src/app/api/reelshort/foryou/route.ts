...
export async function GET(request: NextRequest) {
  try {
    ...
    const rawList = ((json?.data as Record<string, unknown>)?.lists as unknown[]) || [];
    ...
    const mappedData = Array.isArray(rawList)
      ? (rawList as Record<string, unknown>[]).map((item) => ({
          bookId: item.book_id,
    ...
    const filteredData = mappedData.filter((item) => item && item.bookId);
