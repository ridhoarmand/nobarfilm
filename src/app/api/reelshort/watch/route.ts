...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${UPSTREAM_API}/episode?bookId=${encodeURIComponent(bookId)}&episodeNumber=${encodeURIComponent(episodeNumber)}`, {
      cache: 'no-store',
    });

    if (!response.ok) {
      return encryptedResponse({ error: 'Failed to fetch episode' }, response.status);
    }
