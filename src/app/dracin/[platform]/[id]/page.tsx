/* eslint-disable @typescript-eslint/no-explicit-any */ 'use client';import { useMemo } from 'react';
...
export default function DramaPlatformDetailPage() {
      ...
      return {
        detail: drama
          ? {
              ...
              coverOverride: drama.series_cover.includes('.heic') ? `https://wsrv.nl/?url=${encodeURIComponent(drama.series_cover)}&output=jpg` : drama.series_cover,
            }
          : null,
  ...
  const buildWatchHref = (episodeIndex: number, episodeId?: string, isPrimary?: boolean) => {
    switch (platform) {
      case 'dramabox':
        return isPrimary ? `/dracin/watch/dramabox/${id}` : `/dracin/watch/dramabox/${id}?ep=${episodeIndex}`;
      case 'melolo':
        return episodeId ? `/dracin/watch/melolo/${id}/${episodeId}?ep=${episodeIndex}` : '#';
      case 'freereels':
        return `/dracin/watch/freereels/${id}?ep=${episodeIndex}`;
      case 'flickreels':
        return episodeId ? `/dracin/watch/flickreels/${id}/${episodeId}?ep=${episodeIndex}` : '#';
      case 'netshort':
        return `/dracin/watch/netshort/${id}?ep=${episodeIndex}`;
      case 'reelshort':
        return isPrimary ? `/dracin/watch/reelshort/${id}` : `/dracin/watch/reelshort/${id}?ep=${episodeIndex}`;
      default:
        return '#';
    }
  };
  ...
  if (!isPlatformValid) {
    return (
      <div className="min-h-screen pt-24 px-4">
        <UnifiedErrorDisplay title="Platform tidak ditemukan" message="Platform drama tidak dikenali." onRetry={() => router.push('/dracin')} retryLabel="Kembali" />
      </div>
    );
  }
  ...
  if (error || !detail) {
    ...
    return (
      <div className="min-h-screen pt-24 px-4">
        <UnifiedErrorDisplay title="Drama tidak ditemukan" message={`Tidak dapat memuat detail drama. Error: ${errorMessage}`} onRetry={() => router.push('/')} retryLabel="Kembali ke Beranda" />
      </div>
    );
  }
  ...
  const firstEpisodeId = isMelolo ? (episodes as any[])?.[0]?.vid : isFlickReels ? (episodes as any[])?.[0]?.id : undefined;
    ...
    <main className="min-h-screen bg-black">
      <div className="relative min-h-[60vh] md:min-h-[70vh] flex items-end pb-12">
        <div className="absolute inset-0 overflow-hidden">
          <Image src={coverUrl} alt="" fill className="object-cover opacity-30 blur-2xl scale-110" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-8 md:gap-12 items-end">
            <div className="hidden md:block">
              <div className="relative aspect-[2/3] w-full rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10 group">
                <Image src={coverUrl} alt={detail.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
            ...
            <div className="space-y-6 md:pb-4">
              <div className="space-y-4">
                <div className="md:hidden flex justify-center mb-6">
                  <div className="relative w-72 aspect-[2/3] rounded-2xl overflow-hidden border-2 border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-12">
        ...
        <div className="mt-20">
          <DracinSection title="Mungkin Anda Suka" dramas={trendingDramas} isLoading={trendingLoading} />
        </div>
      </div>
    </main>
  );
