...
export default function ReelShortWatchPage() {
  ...
  useEffect(() => {
    ...
    if (epParam) setCurrentEpisode(parseInt(epParam) || 1);
  }, [epParam]);
  ...
  const handleVideoEnded = useCallback(() => {
    ...
    if (currentEpisode < totalEpisodes) {
      ...
      router.replace(`/dracin/watch/reelshort/${bookId}?ep=${nextEp}`, { scroll: false });
    }
  ...
  const goToEpisode = useCallback(
    (ep: number) => {
      ...
      router.replace(`/dracin/watch/reelshort/${bookId}?ep=${ep}`, { scroll: false });
      setShowEpisodeList(false);
    },
    ...
    <main className="fixed inset-0 bg-black flex flex-col overflow-hidden">
      ...
      <div className="relative h-16 flex-shrink-0 z-40 bg-zinc-950/95 backdrop-blur-md border-b border-white/5">
        <div className="relative z-10 flex items-center justify-between h-full px-4 max-w-7xl mx-auto">
          <Link href={`/dracin/reelshort/${bookId}`} className="flex items-center gap-2 text-white/90 hover:text-white transition-colors p-2 -ml-2 rounded-full hover:bg-white/10">
        ...
        <div className="h-full max-h-full max-w-full aspect-[9/16] md:max-w-[480px] relative">
          ...
          {episodeData?.isLocked && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm text-center p-4 z-20">
              ...
              <Link href={`/dracin/reelshort/${bookId}`} className="px-6 py-2 bg-primary text-white rounded-full font-medium hover:bg-primary/90 transition-colors">
                Kembali ke Detail
              </Link>
            </div>
          )}

          {videoSource && <DracinPlayer key={`reelshort-player-${currentEpisode}`} src={videoSource} poster={detailData?.cover} onEnded={handleVideoEnded} autoPlay={autoPlayNext} />}
        </div>
      </div>
